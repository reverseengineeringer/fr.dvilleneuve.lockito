  ...
  implements ChromeDevtoolsDomain
{
  ...
  public DOMStorage(Context paramContext)
  {
    ...
    mDOMStoragePeerManager = new DOMStoragePeerManager(paramContext);
  }
  ...
  {
    mDOMStoragePeerManager.removePeer(paramJsonRpcPeer);
  }
  
  @ChromeDevtoolsMethod
  public void enable(JsonRpcPeer paramJsonRpcPeer, JSONObject paramJSONObject)
  {
    mDOMStoragePeerManager.addPeer(paramJsonRpcPeer);
  }
  ...
  {
    paramJSONObject = (StorageId)mObjectMapper.convertValue(paramJSONObject.getJSONObject("storageId"), StorageId.class);
    paramJsonRpcPeer = new ArrayList();
    Object localObject = securityOrigin;
    if (isLocalStorage)
    {
      paramJSONObject = mContext.getSharedPreferences((String)localObject, 0).getAll().entrySet().iterator();
  ...
  public void removeDOMStorageItem(JsonRpcPeer paramJsonRpcPeer, JSONObject paramJSONObject)
    throws JSONException
  {
    paramJsonRpcPeer = (StorageId)mObjectMapper.convertValue(paramJSONObject.getJSONObject("storageId"), StorageId.class);
    paramJSONObject = paramJSONObject.getString("key");
    if (isLocalStorage) {
      mContext.getSharedPreferences(securityOrigin, 0).edit().remove(paramJSONObject).apply();
    }
  }
    ...
    throws JSONException, JsonRpcException
  {
    ...
    if (isLocalStorage)
    {
      localSharedPreferences = mContext.getSharedPreferences(securityOrigin, 0);
      ...
      if (localObject == null)
      {
        ...
        {
          CLog.writeToConsole(mDOMStoragePeerManager, Console.MessageLevel.ERROR, Console.MessageSource.STORAGE, localDOMStorageAssignmentException.getMessage());
          if (!localSharedPreferences.contains(str)) {
            break label227;
          }
        }
        mDOMStoragePeerManager.signalItemUpdated(paramJsonRpcPeer, str, paramJSONObject, SharedPreferencesHelper.valueToString(localObject));
      }
    }
    else
    {
    ...
    label227:
    mDOMStoragePeerManager.signalItemRemoved(paramJsonRpcPeer, str);
  }
  
  private static class DOMStorageAssignmentException
    extends Exception
  {
