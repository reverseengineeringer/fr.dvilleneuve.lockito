  ...
  extends PendingResult<R>
{
  ...
  public final R await(long paramLong, TimeUnit paramTimeUnit)
  {
    ...
    for (;;)
    {
      zzab.zza(bool1, "await must not be called on the UI thread when time is greater than zero.");
      if (!sP)
      {
        bool1 = true;
        label38:
      ...
      try
      {
        if (!zzalc.await(paramLong, paramTimeUnit)) {
          zzaa(Status.sj);
        }
        zzab.zza(isReady(), "Result is not ready.");
        return get();
  ...
  public final void setResultCallback(ResultCallback<? super R> paramResultCallback, long paramLong, TimeUnit paramTimeUnit)
  {
    ...
    for (;;)
    {
      return;
      sN = paramResultCallback;
      sK.zza(this, paramTimeUnit.toMillis(paramLong));
    }
  }
  ...
  public <S extends Result> TransformedResult<S> then(ResultTransform<? super R, ? extends S> paramResultTransform)
  {
    ...
    for (;;)
    {
      synchronized (sJ)
      {
        ...
        bool1 = bool2;
        zzab.zza(bool1, "Cannot call then() if callbacks are set.");
        sT = true;
        sS = new zzrc(sL);
        paramResultTransform = sS.then(paramResultTransform);
        if (isReady())
        {
          sK.zza(sS, get());
  ...
  protected final void zza(zzr paramzzr)
  {
    synchronized (sJ)
    {
      sR = paramzzr;
    ...
    extends Handler
  {
    ...
    public void zza(zzpt<R> paramzzpt, long paramLong)
    {
      sendMessageDelayed(obtainMessage(2, paramzzpt), paramLong);
    }
