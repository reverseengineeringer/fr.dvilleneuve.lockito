...
public class DataFormatReaders
{
    ...
    throws IOException
  {
    ...
    if (i < j)
    {
      localObject3 = arrayOfObjectReader[i];
      paramAccessorForReader.reset();
      localObject4 = ((ObjectReader)localObject3).getFactory().hasFormat(paramAccessorForReader);
    ...
    for (;;)
    {
      ...
      if ((localObject1 == null) || (((MatchStrength)localObject2).ordinal() < ((MatchStrength)localObject4).ordinal()))
      {
        if (((MatchStrength)localObject4).ordinal() >= _optimalMatch.ordinal()) {
          for (;;)
          {
            return paramAccessorForReader.createMatcher((ObjectReader)localObject3, (MatchStrength)localObject4);
            break;
  ...
  public DataFormatReaders withType(JavaType paramJavaType)
  {
    ...
    while (i < j)
    {
      arrayOfObjectReader[i] = _readers[i].withType(paramJavaType);
      i += 1;
    }
    return new DataFormatReaders(arrayOfObjectReader, _optimalMatch, _minimalMatch, _maxInputLookahead);
  }
